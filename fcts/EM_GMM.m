...
nbData = size(Data,2);%返回Data列数
if ~isfield(model,'params_nbMinSteps')
	model.params_nbMinSteps = 5; %Minimum number of iterations allowed允许的最小迭代数
  
end
if ~isfield(model,'params_nbMaxSteps')
	model.params_nbMaxSteps = 100; %Maximum number of iterations allowed允许的最大迭代数
end
if ~isfield(model,'params_maxDiffLL')
	model.params_maxDiffLL = 1E-4; %Likelihood increase threshold to stop the algorithm似然增加阈值停止算法
end
if ~isfield(model,'params_diagRegFact')
	%model.params.diagRegFact = 1E-8; %Regularization term is optional正则化项是可选的
