...
diagRegularizationFactor = 1E-8; %Optional regularization term可选正则化项

TimingSep = linspace(min(Data(1,:)), max(Data(1,:)), model.nbStates+1);%将数据分成5段

for i=1:model.nbStates%模型中含有nbstate个高斯模型
	idtmp = find( Data(1,:)>=TimingSep(i) & Data(1,:)<TimingSep(i+1));
	model.Priors(i) = length(idtmp);
	model.Mu(:,i) = mean(Data(:,idtmp)');%获取均值
	model.Sigma(:,:,i) = cov(Data(:,idtmp)');%获取方差
	%Optional regularization term to avoid numerical instability可选的正则化项，以避免数值不稳定
